...
import { bufferToHex, deriveKEK, hexToBuffer, decrypt, decryptRSA, generateMasterKey, encrypt, encryptRSA, deriveChunkKey } from "../../utils/crypto";
...
import { startUpload, uploadManifest, handleChunkEncryption } from "../components/UploadFileFeature/uploadFile";
  ...
  try {
    ...
    switch (type) {
      ...
      case "GET_FILE_KEYS": {
        ...
        const user_file_keys = await fetch(`${config.BACKENDURL}/users/file-keys`, {
            method: "GET",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            ...
            return data.data.fileKeysData as { file_id: string; encrypted_file_key: string; encrypted_manifest_key: string }[];
          });

          for (const { file_id, encrypted_file_key, encrypted_manifest_key } of user_file_keys) {
            sessionFileKeys.set(file_id, { encrypted_file_key, encrypted_manifest_key, temp_decrypted_file_key: null });
          }
        result = { success: true };
        break;
      }
      ...
      case "GET_FILE_NAMES": {
        ...
        const decryptedFilesPromise = files.map(async (file) => {
          try {
            const file_master_key = sessionFileKeys.get(file.id)?.encrypted_file_key;
            
            if (!file_master_key) {
              throw new Error("File master key not found in session for file: " + file.id);
            }
      ...
      case "UPLOAD_FILE": {
        ...
        file_id = await startUpload(enc_file_name_data, selectedFile, file_id, enc_file_key, userPublicKey);
        ...
        const {manifest_uuid, wrapped_manifest_key} = await uploadManifest(file_id, manifest, userPublicKey, userPrivateKey);
      ...
      case "GET_AND_DECRYPT_CHUNK": {
        ...
        const file_master_key_encrypted = sessionFileKeys.get(file_id)?.encrypted_file_key;

        if (!file_master_key_encrypted) {
          throw new Error("File master key not found in session for file: " + file_id);
        }
        
        let file_master_key = sessionFileKeys.get(file_id)!.temp_decrypted_file_key;
        ...
        if (!file_master_key) {
          ...
          sessionFileKeys.get(file_id)!.temp_decrypted_file_key = file_master_key;
        }
      ...
      case "SHARE_FILE": {
        ...
        const encrypted_manifest_key = sessionFileKeys.get(payload.fileId)?.encrypted_manifest_key;
        if (!encrypted_manifest_key) {
          throw new Error("File session data not found for file: " + payload.fileId);
        }

        const encrypted_file_key = sessionFileKeys.get(payload.fileId)?.encrypted_file_key;
        if (!encrypted_file_key) {
          throw new Error("File session data not found for file: " + payload.fileId);
        }
      
        await shareFile(file_id, recipient_username, encrypted_file_key, encrypted_manifest_key, userPrivateKey);
        result = { success: true };
        break;
      }
