...
import { getSharedFilesInFolderService } from "../../services/folders/getSharedFilesInFolderService";
import { isUuidV4 } from "../../utils/validators";

export async function getSharedFilesInFolderController(
  ...
  try {
    const files = await getSharedFilesInFolderService(user_id, folder_id);
    res.status(200).json({
    ...
    const message = error instanceof Error ? error.message : "Unable to retrieve shared files";
    ...
    console.error("Get shared files by folder failed:", error);
    res.status(500).json({ message: "Unable to retrieve shared files", success: false });
  }
}

export default getSharedFilesInFolderController;
